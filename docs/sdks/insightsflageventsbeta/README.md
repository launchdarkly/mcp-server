...
* [getFlagEvents](#getflagevents) - List flag events
...
async function run() {
  const result = await launchdarklyMcpServer.insightsFlagEventsBeta.getFlagEvents({
    projectKey: "<value>",
...
import { insightsFlagEventsBetaGetFlagEvents } from "@launchdarkly/mcp-server/funcs/insightsFlagEventsBetaGetFlagEvents.js";
...
async function run() {
  const res = await insightsFlagEventsBetaGetFlagEvents(launchdarklyMcpServer, {
    projectKey: "<value>",
    environmentKey: "<value>",
  });
...
**Promise\<[models.FlagEventCollectionRep](../../models/flageventcollectionrep.md)\>**
