...
async function run() {
  const result = await launchDarkly.insightsPullRequests.list({
    projectKey: "<value>",
  });
...
import { insightsPullRequestsList } from "@launchdarkly/mcp-server/funcs/insightsPullRequestsList.js";
...
async function run() {
  const res = await insightsPullRequestsList(launchDarkly, {
    projectKey: "<value>",
  });
...
**Promise\<[components.PullRequestCollectionRep](../../models/components/pullrequestcollectionrep.md)\>**
