...
const args = {
  request: operations.PatchApplicationRequest$inboundSchema,
};

export const tool$applicationsBetaPatchApplication: ToolDefinition<
  typeof args
> = {
  name: "applications-(beta)-patch-application",
...
Update an application. You can update the \`description\` and \`kind\` fields. Requires a [JSON patch](https://datatracker.ietf.org/doc/html/rfc6902) representation of the desired changes to the application. To learn more, read [Updates](https://launchdarkly.com/docs/api#updates).`,
  ...
  tool: async (client, args, ctx) => {
    const [result, apiCall] = await applicationsBetaPatchApplication(
      client,
      args.request,
