...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PutContextKindRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpsertContextKindPayload, {
    explode: true,
  });
  ...
  const path = pathToFunc("/api/v2/projects/{projectKey}/context-kinds/{key}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.UpsertResponseRep$inboundSchema),
