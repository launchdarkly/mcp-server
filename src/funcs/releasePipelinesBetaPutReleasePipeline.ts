 ...
 */
export function releasePipelinesBetaPutReleasePipeline(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PutReleasePipelineRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateReleasePipelineInput, {
    explode: true,
  });

  const pathParams = {
    pipelineKey: encodeSimple("pipelineKey", payload.pipelineKey, {
      explode: false,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, models.ReleasePipeline$inboundSchema),
