...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetSegmentMembershipForContextRequest$outboundSchema.parse(
        value,
      ),
  ...
  const pathParams = {
    contextKey: encodeSimple("contextKey", payload.contextKey, {
      explode: false,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.BigSegmentTarget$inboundSchema),
