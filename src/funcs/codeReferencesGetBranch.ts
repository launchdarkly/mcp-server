...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetBranchRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    branch: encodeSimple("branch", payload.branch, {
      explode: false,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.BranchRep$inboundSchema),
