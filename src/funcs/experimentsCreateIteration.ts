...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateIterationRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.IterationInput, { explode: true });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.IterationRep$inboundSchema),
