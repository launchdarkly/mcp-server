...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PostMetricRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.MetricPost, { explode: true });
  ...
  const path = pathToFunc("/api/v2/metrics/{projectKey}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.MetricRep$inboundSchema),
