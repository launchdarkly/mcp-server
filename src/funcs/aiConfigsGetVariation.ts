 ...
 */
export function aiConfigsGetVariation(
  client: LaunchDarklyCore,
  request: operations.GetAIConfigVariationRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AIConfigVariationsResponse,
    | errors.ErrorT
    | LaunchDarklyError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAIConfigVariationRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AIConfigVariationsResponse$inboundSchema),
