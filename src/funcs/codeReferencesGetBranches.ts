 ...
 */
export function codeReferencesGetBranches(
  client: LaunchDarklyCore,
  request: operations.GetBranchesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.BranchCollectionRep,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetBranchesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/v2/code-refs/repositories/{repo}/branches")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.BranchCollectionRep$inboundSchema),
