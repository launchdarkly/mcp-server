 ...
 * </details>
 */
export function layersUpdateLayer(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateLayerRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.LayerPatchInput, { explode: true });

  const pathParams = {
    layerKey: encodeSimple("layerKey", payload.layerKey, {
      explode: false,
  ...
  const path = pathToFunc("/api/v2/projects/{projectKey}/layers/{layerKey}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, models.LayerRep$inboundSchema),
