 ...
 */
export function accountUsageGetStreamUsageSDKVersions(
  client: LaunchDarklyCore,
  request: operations.GetStreamUsageSdkversionRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.SDKVersionListRep,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetStreamUsageSdkversionRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/v2/usage/streams/{source}/sdkversions")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.SDKVersionListRep$inboundSchema),
