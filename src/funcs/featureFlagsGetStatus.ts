...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetFeatureFlagStatusAcrossEnvironmentsRequest$outboundSchema
        .parse(value),
  ...
  const path = pathToFunc("/api/v2/flag-status/{projectKey}/{featureFlagKey}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.FeatureFlagStatusAcrossEnvironments$inboundSchema),
