 ...
 */
export function featureFlagsListDependentFlags(
  client: LaunchDarklyCore,
  request: operations.GetDependentFlagsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.MultiEnvironmentDependentFlags,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetDependentFlagsRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.MultiEnvironmentDependentFlags$inboundSchema),
