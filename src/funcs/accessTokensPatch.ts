 ...
 */
export function accessTokensPatch(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PatchTokenRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/v2/tokens/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.Token$inboundSchema),
