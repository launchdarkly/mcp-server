 ...
 * Get context attribute values.
 */
export function contextsGetAttributeValues(
  client: LaunchDarklyCore,
  request: operations.GetContextAttributeValuesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ContextAttributeValuesCollection,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetContextAttributeValuesRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ContextAttributeValuesCollection$inboundSchema),
