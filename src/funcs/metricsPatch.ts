...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PatchMetricRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    metricKey: encodeSimple("metricKey", payload.metricKey, {
      explode: false,
  ...
  const path = pathToFunc("/api/v2/metrics/{projectKey}/{metricKey}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.MetricRep$inboundSchema),
