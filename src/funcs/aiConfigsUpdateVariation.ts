...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PatchAIConfigVariationRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AIConfigVariationPatch, {
    explode: true,
  });
  ...
  const pathParams = {
    ...
    variationKey: encodeSimple("variationKey", payload.variationKey, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AIConfigVariation$inboundSchema),
