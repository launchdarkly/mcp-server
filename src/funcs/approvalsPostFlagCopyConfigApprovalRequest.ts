 ...
 */
export function approvalsPostFlagCopyConfigApprovalRequest(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PostFlagCopyConfigApprovalRequestRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload.createCopyFlagConfigApprovalRequestRequest,
    { explode: true },
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, models.FlagConfigApprovalRequestResponse$inboundSchema),
