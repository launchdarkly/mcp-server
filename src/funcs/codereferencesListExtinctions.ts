 ...
 */
export function codereferencesListExtinctions(
  client: LaunchDarklyCore,
  request: operations.GetExtinctionsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExtinctionCollectionRep,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetExtinctionsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/v2/code-refs/extinctions")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ExtinctionCollectionRep$inboundSchema),
