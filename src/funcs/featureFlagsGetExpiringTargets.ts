 ...
 */
export function featureFlagsGetExpiringTargets(
  client: LaunchDarklyCore,
  request: operations.GetExpiringContextTargetsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ExpiringTargetGetResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetExpiringContextTargetsRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ExpiringTargetGetResponse$inboundSchema),
