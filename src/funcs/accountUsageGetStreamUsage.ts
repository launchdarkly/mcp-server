...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetStreamUsageRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/v2/usage/streams/{source}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.SeriesListRep$inboundSchema),
