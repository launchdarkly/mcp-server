 ...
 */
export function aiConfigsAddRestrictedModels(
  client: LaunchDarklyCore,
  request: operations.PostRestrictedModelsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.RestrictedModelsResponse,
    | errors.ErrorT
    | errors.ErrorT
    | APIError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PostRestrictedModelsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.RestrictedModelsRequest, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.RestrictedModelsResponse$inboundSchema),
