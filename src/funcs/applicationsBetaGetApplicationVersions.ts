 ...
 */
export function applicationsBetaGetApplicationVersions(
  client: LaunchdarklyMcpServerCore,
  request: operations.GetApplicationVersionsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    models.ApplicationVersionsCollectionRep,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetApplicationVersionsRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    applicationKey: encodeSimple("applicationKey", payload.applicationKey, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/api/v2/applications/{applicationKey}/versions")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, models.ApplicationVersionsCollectionRep$inboundSchema),
