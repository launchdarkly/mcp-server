 ...
 */
export function scheduledChangesPostFlagConfigScheduledChanges(
  client: LaunchdarklyMcpServerCore,
  request: operations.PostFlagConfigScheduledChangesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    models.FeatureFlagScheduledChange,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PostFlagConfigScheduledChangesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.PostFlagScheduledChangesInput, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, models.FeatureFlagScheduledChange$inboundSchema),
