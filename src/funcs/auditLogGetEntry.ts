...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetAuditLogEntryRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/v2/auditlog/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.AuditLogEntryRep$inboundSchema),
