...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PatchCustomRoleRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    customRoleKey: encodeSimple("customRoleKey", payload.customRoleKey, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/api/v2/roles/{customRoleKey}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.CustomRole$inboundSchema),
